...
PATH_PROJECT="http://localhost:8080/film/"
  ...
  public listFilm():Observable<Films[]>{
    return this.httpClient.get<Films[]>(`${this.PATH_PROJECT+"allForClient"}`)
  }
  ...
  public getFilmById(id:number):Observable<Films>{
    return this.httpClient.get<Films>(`${this.PATH_PROJECT+"byIdForClient/"+id}`)
  }
  public byCat(id:number):Observable<Films[]>{
    return this.httpClient.get<Films[]>(`${this.PATH_PROJECT+"bycat/"+id}`)
  }
}
